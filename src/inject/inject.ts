...
export default class TWExtension {
	...
	async _onEncryptButtonClick(event: MouseEvent): Promise<void> {
		...
		const textarea =
			document.querySelector<HTMLTextAreaElement>("textarea#message");
		if (!textarea) {
			return;
		}
		if (
			...
			const str = textarea.value.substring(
	...
	async _decode(): Promise<void> {
		...
		for (const { element, raw } of this._params()) {
			...
			const parentCodeBoxP =
				newCode?.parentElement?.parentElement?.querySelector("p");
	...
	_blur(elem: HTMLElement): void {
		elem.animate([{ filter: "blur(5px)" }, { filter: "none" }], {
			duration: 300,
		});
	}
